...
def chuyen_chuoi_sang_so(chuoi, number_skip_hang,number_skip_vi):
    
    numbers = extract_numbers(chuoi)
    
    number_array = list(range(numbers[0], numbers[1] + 1))

    lst_number_skip_hang = list(str(number_skip_hang))
    lst_number_skip_vi = list(str(number_skip_vi))
    ...
    for _number_skip_vi in lst_number_skip_vi:
        filtered_numbers = [num for num in filtered_numbers if int(num) % 10 != int(_number_skip_vi)]
...
async def handlekeyword_BOHANGVABOVI (update: Update, context: ContextTypes.DEFAULT_TYPE):
    print("async def handlekeyword_BOHANGVABOVI (update: Update, context: ContextTypes.DEFAULT_TYPE):")
    ...
    input_str = re.sub(r'(bo\s*hang|bohang|bh|hang+)(\d+)', r'|h\2', input_str)

    input_str = re.sub(r'(bo\s*vi|bovi|bv|vi+)(\d+)', r'|v\2', input_str)
    ...
    if len(input_str) >=2:
        ...
        number_skip_first = input_str[len(input_str)-1]

        number_skip_seccond = input_str[len(input_str)-2]


        if number_skip_first.startswith('h'): 
            number_skip_hang = int(number_skip_first.replace('h',''))

        else:

            number_skip_vi = int(number_skip_first.replace('v',''))

        
        if number_skip_seccond.startswith('h'): 

            number_skip_hang = int(number_skip_seccond.replace('h',''))

        else:

            number_skip_vi = int(number_skip_seccond.replace('v',''))
        

        print(number_skip_hang)


        print(number_skip_vi)
        ...
        for item in format_array_muns:
            ...
            if len(item['so']) == 0 :
                ...
                for  so in item['so']:
                    ...
                    output_str_format = chuyen_chuoi_sang_so(so,number_skip_hang, number_skip_vi)
