...
async def handlekeyword_TRON(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in format_array_muns:
        ...
        dai = item['dai']
        random_so = ' '.join(random.sample(item['so'], len(item['so'])))
        kieu = item['kieu']
        ...
        if item['kieu']== '' :
            ...
            if item['dai']== '' :
                formatted_str += f"{random_so} {kieu}; "
            else:
                formatted_str += f"{dai} {random_so} {kieu}; "
